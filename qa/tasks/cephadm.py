...
def ceph_log(ctx, config):
    ...
    try:
        ...
        def first_in_ceph_log(pattern, excludes, only_match):
            ...
            # grep/egrep semantics:
            #   0 => match found
            #   1 => no match
            #   2 => error
            #
            # In a pipeline, the shell may not preserve grep's exact status,
            # but "no stdout and no stderr" is still safely "no match".
            if not stderr:
                return None

            # If stderr exists, this is not a log match. Surface it as a scan error.
        ...
        try:
            ...
            # Important: a log scan failure is not the same as a log hit.
            log.warning('Unable to scan cluster log safely: %s', e)

            # Preserve a more specific/root-cause failure reason if one already exists.
            if 'failure_reason' not in ctx.summary:
