...
from .secret_store import SecretRecord, SecretStoreMon
...
class SecretMgr:
    ...
    def get_value(self, ref: SecretRef) -> Any:
        ...
        # Defensive (shouldn't happen since SecretRecord enforces dict)
        return rec.data
    ...
    def ls(
        ...
        scope: Optional[SecretScope] = SecretScope.GLOBAL,
        target: Optional[str] = None,
    ) -> List[SecretRecord]:
        ...
        def _scan(v: Any) -> None:
            if isinstance(v, dict):
                ...
                for m in _SECRET_URI_RE.finditer(v):
                    uri = m.group(0)
                    try:
                        refs.add(parse_secret_uri(uri, namespace))
    ...
    def resolve_object(self, obj: Any, namespace: str) -> Any:
        ...
        def _resolve_str(s: str) -> Any:
            ...
            if s_strip.startswith(SECRET_URI_SCHEME) and _SECRET_URI_RE.fullmatch(s_strip):
                ref = parse_secret_uri(s_strip, namespace=namespace)
            ...
            if SECRET_URI_SCHEME in s:
                ...
                def repl(m: re.Match) -> str:
                    uri = m.group(0)
                    ref = parse_secret_uri(uri, namespace=namespace)
                    val = self.get_value(ref)
                    if not isinstance(val, str):
                        # This is where we enforce: >1 keys with no ?key= is not ok for strings.
                        raise CephSecretException(
                            f"Secret {ref.to_uri()} resolved to non-string; cannot embed into string. "
