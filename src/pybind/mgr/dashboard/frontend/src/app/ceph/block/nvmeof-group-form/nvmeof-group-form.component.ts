...
export class NvmeofGroupFormComponent extends CdForm implements OnInit {
  ...
  currentCertificate: CephServiceCertificate | null = null;
  ...
  onSubmit() {
    ...
    const serviceId = this.resolveServiceId(groupName);
    const serviceName =
   ...
   * Create keeps the target-branch service_id (`nvmeof.<group>`).
   * Edit reuses the existing service_id so updates do not rename the service.
   */
  private resolveServiceId(groupName: string): string {
