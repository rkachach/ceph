    ...
    def _disabled_daemon_should_start(
        ...
        if svc.ranked(spec):
            same_rank_daemons = [
                ...
                and d.hostname and not self.mgr.cache.is_host_unreachable(d.hostname)
            ]
            if any(
        ...
        for dd in daemons:
            ...
            if not last_config:
                ...
                if self.mgr.upgrade.upgrade_state is not None and not self.mgr.upgrade.upgrade_state.paused:
                    logger.debug('Skipping reconfig of %s for monmap change (upgrade in progress)', dd.name())
                else:
            ...
            with set_exception_subject('service', orchestrator.DaemonDescription(
                ...
                try:
                    ...
                    if daemon_spec.daemon_type != 'agent':
                        ...
                        # (including agent.json) before restart, so rc == 0 means the
                        ...
                        self.log.warning(
                            ...
                            f"exited with code {rc}; leaving agent config deps unchanged so "
